...
class ERTriagePilot:
    def __init__(self):
        ...
        # Structurally Enforced: Private attribute
        self._baseline = {
            'Emergent': 0.3,
            'Urgent': 0.5,
            'Non-Urgent': 0.2
        ...
        self._baseline_items = list(self._baseline.items())

        # Structurally Enforced: Private attribute.
        # Direct access is forbidden; must go through admit_patient.
        self._current_counts = {k: 0 for k in self._baseline}
    ...
    @property
    def baseline(self):
        """Read-only view of the baseline."""
        return self._baseline.copy()

    @property
    def current_counts(self):
        """Read-only view of current counts to prevent external mutation."""
        return self._current_counts.copy()

    def _recalculate_drift(self):
        """
        Internal helper to update the drift metric.
        Called after any change to _current_counts or total_patients.
        """
    ...
    def admit_patient(self, category):
        ...
        self._current_counts[category] += 1
    ...
    def get_current_distribution(self):
        if self.total_patients == 0:
            return {k: 0 for k in self._baseline}
        return {k: v / self.total_patients for k, v in self._current_counts.items()}
